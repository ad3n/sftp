...
// example invokation (darwin): gofmt -w `find . -name \*.go` && (cd server_standalone/ ; go build -tags debug) && go test -tags debug github.com/ad3n/sftp -integration -v -sftp /usr/libexec/sftp-server -run ServerCompareSubsystems
